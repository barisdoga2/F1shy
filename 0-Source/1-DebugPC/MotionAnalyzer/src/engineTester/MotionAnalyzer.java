...
public class MotionAnalyzer {
	...
	public void dataReceived(String msg) {
		try {
			...
			if(msg.contains("ypr")) {
				String[] tokens = msg.split(" ");
				float yaw = Float.parseFloat(tokens[1]);
				float pitch = Float.parseFloat(tokens[2]);
				float roll = Float.parseFloat(tokens[3]);
			...
			if(msg.contains("quat")) {
				String[] tokens = msg.split(" ");
				try {
					float w = Float.parseFloat(tokens[1]);
